...
class UserServiceClient {
   ...
   * @param {string} authToken - Authentication token (optional, for internal service calls)
   * @returns {Promise<Object>} User profile data
   */
  async getUserProfile(userId, authToken = null) {
    try {
      const headers = {};
      if (authToken) {
      ...
      // For internal service calls, we might need to use a different endpoint
      // or pass the user ID as a parameter. Let me check the user routes first.
      const response = await this.makeRequest(`/api/users/profile/${userId}`, 'GET', null, headers);
      return response;
    } catch (error) {
