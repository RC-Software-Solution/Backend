...
exports.createOrder = async (req, res)=> {
    ...
    try {
        ...
        if (unpaidOrders > 0) {
            return res.status(400).json({ message: 'You have an unpaid order' });
        }
        ...
        for(const item of items){
            await Meal_Session.increment('current_orders', { by: item.quantity, where: { meal_time: item.meal_time, date: sequelize.literal("CURDATE()")}});
        }

        broadcastMealSessionUpdate(mealSession);
