...
class OrderServiceClient {
   ...
   */
  async getOrdersByArea(areaId, mealTime = null, date = null, paymentStatus = 'pending') {
    try {
      ...
      console.log(`Fetching orders for area ${areaId} from ${this.baseUrl}/api/orders?${queryParams}`);
      const response = await this.makeRequest(`/api/orders?${queryParams}`, 'GET');
      return response;
    } catch (error) {
  ...
  async updatePaymentStatus(orderId, paymentStatus) {
    try {
      ...
      const response = await this.makeRequest(`/api/orders/${orderId}/payment`, 'PUT', {
